...
async function getCurrentStockDrugWise(facilityId, itemId, stockStatus) {
    // NEW: If no itemId/stockStatus filter, use the facility batch-level query grouped by item
    // This is the new item-grouped version
    let query = '';
    let params = { facilityId };

    if (itemId) {
        query = `
            Select DISTINCT 0 as SlNo,m1.ItemID,m1.multiple,m1.unitcount,m1.ItemCode,m1.ItemName,m2.StkStCode as ItemLevel,m3.VENCode as VEN,
            m1.Strength1 as Strength,m1.Unit as SKU,m4.ItemTypeCode as ItemType,m4.ItemTypeName,m1.ActiveFlag,
            (NVL(cs.ISSUEQTY,0)) CumlMon6,(NVL(cs.CurStock,0)) CurBal,(NVL(cs.CurStock,0)) as CurBalN, (NVL(cs.RESERVED,0))  ReservedQty,(NVL(cs.WARDRETRUN,0))  WardReturnQty
            From masItems m1
            Left Outer Join masItemLevels m2 on (m2.StockStatusID=m1.StockStatusID)
            Left Outer Join masItemVENs m3 on (m3.VENID=m1.VENID)
            Left Outer Join masItemTypes m4 on (m4.ItemTypeID=m1.ItemTypeID)
            Left Outer Join V_FacilityStock1 cs on (cs.ItemID=m1.ItemID and cs.FacilityID=:facilityId)
            Where 1=1 and m1.ItemID=:itemId
        `;
        params.itemId = itemId;
    } else {
        if (stockStatus === 'stock available') {
            query = `
                Select DISTINCT 0 as SlNo,m1.ItemID,m1.multiple,m1.unitcount,m1.ItemCode,m1.ItemName,
                m1.Strength1 as Strength,m1.Unit as SKU,m4.ItemTypeCode as ItemType,m4.ItemTypeName,
                (NVL(cs.ISSUEQTY,0)) CumlMon6,(NVL(cs.CurStock,0)) CurBal,(NVL(cs.CurStock,0)) as CurBalN, (NVL(cs.RESERVED,0))  ReservedQty,(NVL(cs.WARDRETRUN,0))  WardReturnQty
                From vmasItems m1
                Left Outer Join masItemTypes m4 on (m4.ItemTypeID=m1.ItemTypeID)
                Left Outer Join V_FacilityStock1 cs on (cs.ItemID=m1.ItemID and cs.FacilityID=:facilityId)
                Where 1=1 and (NVL(cs.CurStock,0)) > 0
            `;
        } else if (stockStatus === 'stock out') {
            query = `
                Select DISTINCT 0 as SlNo,m1.ItemID,m1.multiple,m1.unitcount,m1.ItemCode,m1.ItemName,m2.StkStCode as ItemLevel,m3.VENCode as VEN,
                m1.Strength1 as Strength,m1.Unit as SKU,m4.ItemTypeCode as ItemType,m4.ItemTypeName,m1.ActiveFlag,
                (NVL(cs.ISSUEQTY,0)) CumlMon6,(NVL(cs.CurStock,0)) CurBal,(NVL(cs.CurStock,0)) as CurBalN, (NVL(cs.RESERVED,0))
                ReservedQty,(NVL(cs.WARDRETRUN,0))  WardReturnQty
                From masItems m1
                inner join V_FACILITYUSESITEM v on v.itemid= m1.itemid
                Left Outer Join masItemLevels m2 on (m2.StockStatusID=m1.StockStatusID)
                Left Outer Join masItemVENs m3 on (m3.VENID=m1.VENID)
                Left Outer Join masItemTypes m4 on (m4.ItemTypeID=m1.ItemTypeID)
                Left Outer Join V_FacilityStock1 cs on (cs.ItemID=m1.ItemID and cs.FacilityID=:facilityId) 
                Where 1=1 and v.facilityid =:facilityId and (NVL(cs.CurStock,0)) < 1
                order by itemname
            `;
        } else {
            // 'all'
            query = `
                ...
                Left Outer Join masItemTypes m4 on (m4.ItemTypeID=m1.ItemTypeID)
                Left Outer Join V_FacilityStock1 cs on (cs.ItemID=m1.ItemID and cs.FacilityID=:facilityId) 
                Where 1=1 and v.facilityid =:facilityId
                order by itemname
            `;
        }
    }
